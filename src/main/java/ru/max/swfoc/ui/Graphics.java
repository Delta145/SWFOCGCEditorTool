...
public class Graphics implements IGraphics {
    ...
    public void launchUI(Display display, Shell shell) {
        ...
        for (int i = 0; i < gameEntities.getCampaigns().size(); i++)
            if (gameEntities.getCampaigns().get(i).getName() != null)
                listOfCampaigns.add(gameEntities.getCampaigns().get(i).getName()
        ...
        for (int i = 0; i < gameEntities.getPlanets().size(); i++)
            if (gameEntities.getPlanets().get(i).getName() != null)
                allPlanets.add(gameEntities.getPlanets().get(i).getName());
            else allPlanets.add(String.format("XML Name: (%s)", gameEntities.getPlanets().get(i).getXmlName()));
            ...
            public void widgetSelected(SelectionEvent event) {
                ...
                try {
                    ...
                    if (campaign.isEdited()) {
                        ...
                        isEdited.select(isEdited.indexOf(Boolean.toString(isEdit)));
                    ...
                    for (int k = 0; k < campaign.getLocations().size(); k++) {
                        if (campaign.getLocations().get(k).getName() != null)
                            camPlanetsList.add(campaign.getLocations().get(k).getName());
                    ...
                    for (int k = 0; k < campaign.getHumanVictoryConditions().size(); k++) {
                        if (campaign.getHumanVictoryConditions().get(k) != null)
                            humanVictoryConditions.add(campaign.getHumanVictoryConditions().get(k));
                    }
                    ...
                    for (int k = 0; k < campaign.getAiVictoryConditions().size(); k++) {
                        if (campaign.getAiVictoryConditions().get(k) != null)
                            aiVictoryConditions.add(campaign.getAiVictoryConditions().get(k));
                    }
        ...
        allPlanets.addSelectionListener(new SelectionListener() {    //��� ������� �� ������� ������ ������
            public void widgetSelected(SelectionEvent event) {
                ...
                try {
                    extractedMethod(selectedItems, allPlanets);
                } catch (ArrayIndexOutOfBoundsException e) {
                    System.out.println("����� �� ������� ������ ������");
                }
            }
        ...
        camPlanetsList.addSelectionListener(new SelectionListener() {    //��� ������� �� ������� ������ ���� ������
            public void widgetSelected(SelectionEvent event) {
                ...
                try {
                    extractedMethod(selectedItems, camPlanetsList);
                } catch (ArrayIndexOutOfBoundsException e) {
                    System.out.println("����� �� ������� ������ ������ ��������");
                }
            }
    ...
    private void extractedMethod(int[] selectedItems, List allPlanets) {
        String planetstring = allPlanets.getItem(selectedItems[0]);
        MPlanet planet = null;
        for (int i = 0; i < gameEntities.getPlanets().size(); i++) {
            if (gameEntities.getPlanets().get(i).getName() != null)
                if (gameEntities.getPlanets().get(i).getName().equals(planetstring))
                    planet = gameEntities.getPlanets().get(i);
        }
        ...
        if (campaign.getSpecialCaseProduction().containsKey(planet)) {
            ...
            for (FactionUnit factionUnit : fu)
                specialCaseProduction.add(factionUnit.getOwner().getName() + " - " + factionUnit.getUnit().getName());
        ...
        if (campaign.getStartingForces().containsKey(planet)) {
            ...
            for (FactionUnit factionUnit : fu)
                startingForces.add(factionUnit.getOwner().getName() + " - " + factionUnit.getUnit().getName());
