...
//	private ScrolledComposite listOfPlanets;
	...
	private ScrolledComposite sccampaigns;	//ложитс€ под listOfCampaigns дл€ возможности прокрутки
	private ScrolledComposite scrCamPlanets;	//ложитс€ под camPlanetsList дл€ возможности прокрутки
	private ScrolledComposite scHumanVictoryConditions;	//ложитс€ под humanVictoryConditions дл€ возможности прокрутки
	private ScrolledComposite scAiVictoryConditions;	//ложитс€ под aiVictoryConditions дл€ возможности прокрутки
	private ScrolledComposite scSpecialCaseProduction;	//ложитс€ под specialCaseProduction дл€ возможности прокрутки
	private ScrolledComposite scStartingForces;	//ложитс€ под startingForces дл€ возможности прокрутки
	private ScrolledComposite scAllPlanets;	//ложитс€ под allPlanets дл€ возможности прокрутки
	private ScrolledComposite scCamTradeRoutes;	//ложитс€ под camTradeRoutesList дл€ возможности прокрутки
	private ScrolledComposite scForProperties;	//ложитс€ под compositeForProperties дл€ возможности прокрутки
	...
	private Composite compositeForProperties;	//самый правый большой блок дл€ большинства свойств кампании
	Composite factionPropertiesComposite = null;	//это блок внутри compositeForProperties который показывает таблицу с фракци€ми и некоторыми свойствами
	Text[] aicontrol;		
	Text[] startingcred;
	Text[] startingtech;
	Text[] maxtech;
	...
	//	UIToolMenu uitoolmenu = new UIToolMenu(shell);
		...
		Composite [] first3CompositesHorizontal = new Composite[3];	//деление окна на 3 части, права€ дл€ свойств
		for(int i = 0; i < 3; i++) {
		...
		Composite[] compositesOfSecondSash = new Composite[2];	//деление серединной части на две вертикальные, сверху будут планеты кампании, торговые пути и спец постройки, юниты планеты, снизу планируетс€ окно дл€ редактировани€ кампании
		for(int i = 0; i < 2; i++) {
		...
		Composite[] compositesOfTopCenterSash = new Composite[2];	//деление серединной верхней части на левую - планеты кампании и правую что далее разделитс€ на тороговые пути, юниты и спец постройки
		for(int i = 0; i < 2; i++) {
		...
		Composite[] compositesVerticalOfTopCenterRight = new Composite[2];	//деление серединной верхней части что осталась справа после предыдущего делени€ на нижнюю дл€ юнитов и верхнюю дл€ последущего делени€ на торговые пути и спецпостройки
		for(int i = 0; i < 2; i++) {
		      ...
		      public void widgetSelected(SelectionEvent event)  {
			       ...
			       campaign = null;
			        ...
			        if (campaign.isEdited()) {
			        	...
			        	System.out.println("¬ышел за пределы списка кампаний");
			        }
		      ...
		      public void widgetSelected(SelectionEvent event)  {
		    	  ...
		    	  specialCaseProduction.removeAll();
				    ...
				    scSpecialCaseProduction.setMinSize(specialCaseProduction.computeSize(SWT.DEFAULT, SWT.DEFAULT));   
		    		  ...
		    		  System.out.println("¬ышел за пределы списка планет");
		    	  }
		   }
		    	 ...
		    	 try {
		    	  ...
		    	  specialCaseProduction.removeAll();
				    ...
				    scSpecialCaseProduction.setMinSize(specialCaseProduction.computeSize(SWT.DEFAULT, SWT.DEFAULT));   
		    		 ...
		    		 System.out.println("¬ышел за пределы списка планет кампании");
		    	 }
		      }
			@Override
			public void widgetDefaultSelected(SelectionEvent arg0) {
